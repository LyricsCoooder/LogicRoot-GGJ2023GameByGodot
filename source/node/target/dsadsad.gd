...
func fin_chenk():
	if not get_parent().get_node("end").node.empty():
		if node.state == true and get_parent().get_node("end").node[0].state and get_parent().get_node("end2").node[0].state == false:
			fin0.visible = true
		if node.state == false and get_parent().get_node("end").node[0].state ==false and get_parent().get_node("end2").node[0].state:
			fin1.visible = true
